...
def readDetails(contract_type):
    ...
    compiled_contract_path = 'EHR.json' #EHR contract code
    deployed_contract_address = '0xcC38b8fe518ab018eb227c3DF39fB24795eDd0a8' #hash address to access EHR contract
...
def saveDataBlockChain(currentData, contract_type):
    ...
    web3 = Web3(HTTPProvider(blockchain_address))
    web3.eth.defaultAccount = web3.eth.accounts[0]
    compiled_contract_path = 'EHR.json' #crowd funding contract file
    deployed_contract_address = '0xcC38b8fe518ab018eb227c3DF39fB24795eDd0a8' #contract address
    with open(compiled_contract_path) as file:
        contract_json = json.load(file)  # load contract info as JSON
        contract_abi = contract_json['abi']  # fetch contract's abi - necessary to call its functions
    file.close()
...
def AddHealthAction(request):
    if request.method == 'POST':
        ...
        filename = fs.save('SecureEHRApp/static/reports/'+fname, myfile)
